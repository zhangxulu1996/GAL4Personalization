...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        if add:
            ...
            info = json.load(open(os.path.join(self.instance_data_root, "init.json"))) # list
        ...
        self.inverted_prompt = info[0]["instance_prompt"]

    def get_style_descriptor(self):
        return "in " + self.instance_prompt[0].split(" in ")[-1]
    ...
    def __getitem__(self, index):
        ...
        example["inverted_prompt"] = self.inverted_prompt
...
def collate_fn(examples):
    ...
    inverted_prompt = [example["inverted_prompt"] for example in examples]
...
def start_new_train(args):
    ...
    if current_round > 1:     
        if args.feedback == "human":   
            ...
            write_feedback_to_json(args, train_dataset.get_style_descriptor(), args.feedback)
    ...
    for epoch in range(0, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                # unet = UNet2DConditionModel.from_pretrained(os.path.join(args.output_dir.replace('4', '3').replace('2d', '3d'), 'checkpoint-500', 'unet')).to(accelerator.device)
                # max_round_reached = generate_new_images(text_encoder, tokenizer, unet, vae, args, accelerator, weight_dtype, evaluator, style_descriptor=train_dataset.get_style_descriptor(), batch=batch)
                # return True
                ...
                timesteps_sch = noise_scheduler.timesteps
                t = random.randint(0, noise_scheduler.config.num_train_timesteps)
                timesteps = timesteps_sch[t]
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    if global_step % 100 == 0 and args.log == True:
                        log_validation(text_encoder, tokenizer, unet, vae, args, accelerator, weight_dtype, train_dataset.get_style_descriptor(), global_step)
...
prompts_all = ["A tie {}", "candy {}", "A drum {}", "A koala {}", "Beach scene {}", "A shoe {}",
            "A pencil {}", "A kite {}", "A cheese {}", "A backpack {}", "A donut {}", "A leaf {}", 
            "Moon {}", "A jacket {}", "A parrot {}", "A lantern {}", "A cluster of grapes {}", "A knife {}"]
...
def log_validation(text_encoder, tokenizer, unet, vae, args, accelerator, weight_dtype, style_descriptor, step):
    logger.info(
    ...
    prompt = args.validation_prompt.format(style_descriptor)
...
def generate_new_images(text_encoder, tokenizer, unet, vae, args, accelerator, weight_dtype, evaluator, style_descriptor, batch):
    ...
    src_txt_features = evaluator.get_text_features(batch["inverted_prompt"][0])
    ...
    for i, prompt in enumerate(prompts):
        prompt = prompt.format(style_descriptor)
        logger.info(
        ...
        with torch.autocast("cuda"):
            for kk in range(gen_num):
                ...
                sim_img_src_txt = evaluator.txt_to_img_similarity(image, text_features=src_txt_features).cpu().numpy()
